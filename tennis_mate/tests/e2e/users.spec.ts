...
async function ensureLoggedIn(page: any) {
  ...
  try {
    await page.waitForURL((url: URL) => url.pathname.startsWith('/login'), { timeout: 5000 });
  ...
  if (!redirectedToLogin) {
    ...
    const onUsersPage = await page.getByRole('heading', { name: 'ユーザーマスタ' }).isVisible();
    if (onUsersPage) {
      return;
    }
    // Give an extra short window for delayed redirect
    try {
      await page.waitForURL((url: URL) => url.pathname.startsWith('/login'), { timeout: 3000 });
      redirectedToLogin = true;
    } catch {
      redirectedToLogin = false;
    }
...
test('admin users CRUD', async ({ page }) => {
  ...
  // Reset PW
  await row.locator('xpath=..').getByRole('button', { name: 'PWリセット' }).click();
...
test('shows success message on create', async ({ page }) => {
  ...
  await expect(page.getByText('ユーザーを作成しました')).toBeVisible({ timeout: 5000 });
...
test('shows success message on update', async ({ page }) => {
  ...
  // Check for success message
  await expect(page.getByText('ユーザーを更新しました')).toBeVisible({ timeout: 5000 });
...
test('shows success message on password reset', async ({ page }) => {
  ...
  // Check for success message
  await expect(page.getByText('パスワードをリセットしました')).toBeVisible({ timeout: 5000 });
...
test('shows success message on delete', async ({ page }) => {
  ...
  // Check for success message
  await expect(page.getByText('ユーザーを削除しました')).toBeVisible({ timeout: 5000 });
});
...
test('can create user with all roles', async ({ page }) => {
  ...
  for (const role of roles) {
    ...
    await page.getByRole('button', { name: '新規作成' }).click();
    await page.getByLabel('Email').fill(email);
    await page.locator('label:has-text("ロール")').locator('..').locator('div[role="combobox"]').click();
...
test('shows empty state when no users exist', async ({ page }) => {
  ...
  const emptyMessage = page.getByText('ユーザーが登録されていません', { exact: false });
