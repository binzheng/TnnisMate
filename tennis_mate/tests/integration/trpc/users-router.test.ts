...
describe("tRPC users router (integration)", () => {
		...
		const db = {
			user: {
				...
				create: async ({ data }: any) => {
					const id = `u${store.length + 1}`;
					const row = { id, ...data };
					store.push(row);
					return row;
				},
				update: async ({ where, data }: any) => {
					const i = store.findIndex((u) => u.id === where.id);
	...
	it("operator can list but not create", async () => {
		const db = {
			user: {
				findUnique: async () => ({ id: "me", role: "operator" }),
				findMany: async () => [],
				create: async () => ({}),
			},
		};
		...
		await expect(
			caller.users.create({ email: "x@y", password: "aA12345" }),
		).rejects.toBeTruthy();
	});
	...
	it("validates email format on create", async () => {
		...
		const caller = createCaller(makeCtx("admin", db) as any);
		await expect(
			caller.users.create({ email: "invalid-email", password: "pass1234" }),
		).rejects.toThrow();
	});
	...
	it("validates password length on create (min 6)", async () => {
		...
		const caller = createCaller(makeCtx("admin", db) as any);
		await expect(
			caller.users.create({ email: "test@example.com", password: "12345" }),
		).rejects.toThrow();
	});
	...
	it("validates password length on reset (min 6)", async () => {
		...
		const caller = createCaller(makeCtx("admin", db) as any);
		await expect(
			caller.users.resetPassword({ id: "u1", password: "12345" }),
		).rejects.toThrow();
	});
	...
	it("update validates email format", async () => {
		...
		const caller = createCaller(makeCtx("admin", db) as any);
		await expect(
			caller.users.update({ id: "u1", email: "invalid-email" }),
		).rejects.toThrow();
	});
	...
	it("player cannot create user", async () => {
		...
		const caller = createCaller(makeCtx("player", db) as any);
		await expect(
			caller.users.create({ email: "test@example.com" }),
		).rejects.toThrow();
	});
	...
	it("coach cannot update user", async () => {
		...
		const caller = createCaller(makeCtx("player", db) as any);
		await expect(
			caller.users.update({ id: "u1", name: "New Name" }),
		).rejects.toThrow();
	});
	...
	it("creates user with all roles", async () => {
		...
		const player = await caller.users.create({
			email: "player@example.com",
			role: "player",
		});
		const coach = await caller.users.create({
			email: "coach@example.com",
			role: "coach",
		});
		const operator = await caller.users.create({
			email: "operator@example.com",
			role: "operator",
		});
		const admin = await caller.users.create({
	...
	it("remove returns ok status", async () => {
		const store: any[] = [
			{ id: "u1", email: "test@x", name: "Test", role: "player" },
		];
		const db = {
			user: {
				findUnique: async ({ where }: any) => ({ id: where.id, role: "admin" }),
