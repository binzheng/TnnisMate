...
import { ListUsersUseCase, CreateUserUseCase, UpdateUserUseCase, DeleteUserUseCase, ResetPasswordUseCase } from '@/server/core/usecases/users/usecases';
import type { UsersRepository, CreateUserInput, UpdateUserInput, UserDto } from '@/server/core/usecases/users/ports';
...
class InMemoryUsersRepo implements UsersRepository {
  ...
  async list(): Promise<UserDto[]> { return this.users.slice(); }
  async create(input: CreateUserInput & { passwordHash?: string | undefined; }): Promise<UserDto> {
    const id = `u${this.users.length + 1}`;
    const row: UserDto = { id, email: input.email, name: input.name, role: (input.role ?? 'player') as any };
    this.users.push(row);
  ...
  async delete(id: string): Promise<void> { this.users = this.users.filter(u => u.id !== id); }
  async setPassword(id: string, passwordHash: string): Promise<void> { this.pw[id] = passwordHash; }
}

class AuthzAllow implements AuthzPort { async getRole() { return 'admin'; } async requireRole() { /* ok */ } }
class AuthzOp implements AuthzPort { async getRole() { return 'operator'; } async requireRole(_uid: string, allowed: any[]) { if (!allowed.includes('operator') && !allowed.includes('admin')) throw new Error('forbidden'); } }
class AuthzDeny implements AuthzPort { async getRole() { return 'player'; } async requireRole() { throw new Error('forbidden'); } }
...
describe('Users usecases', () => {
  ...
  it('admin can create/update/delete and reset password with audit', async () => {
    ...
    const u = await create.execute('admin', { email: 'b@example.com', name: 'B', role: 'coach', passwordHash: 'hash' });
    expect(u.id).toBeDefined();
    expect(repo.pw[u.id]).toBe('hash');

    const u2 = await update.execute('admin', { id: u.id, name: 'B2' } as UpdateUserInput);
    ...
    expect(keys).toEqual(expect.arrayContaining(['user.create','user.update','user.password.reset','user.delete']));
  });

  it('create user with default role', async () => {
    const repo = new InMemoryUsersRepo();
    const create = new CreateUserUseCase(repo, new AuthzAllow());
    const u = await create.execute('admin', { email: 'test@example.com', name: 'Test' });
  ...
  it('update user email', async () => {
    ...
    const u = await create.execute('admin', { email: 'old@example.com', name: 'User' });
    const updated = await update.execute('admin', { id: u.id, email: 'new@example.com' });
  ...
  it('update user role', async () => {
    ...
    const u = await create.execute('admin', { email: 'test@example.com', role: 'player' });
  ...
  it('update non-existent user throws error', async () => {
    const repo = new InMemoryUsersRepo();
    const update = new UpdateUserUseCase(repo, new AuthzAllow());
    await expect(update.execute('admin', { id: 'non-existent', name: 'Test' })).rejects.toThrow('not found');
  });
  ...
  it('password reset updates password hash', async () => {
    ...
    const u = await create.execute('admin', { email: 'test@example.com', passwordHash: 'old-hash' });
  ...
  it('all roles can be created', async () => {
    ...
    const player = await create.execute('admin', { email: 'player@x', role: 'player' });
    const coach = await create.execute('admin', { email: 'coach@x', role: 'coach' });
    const operator = await create.execute('admin', { email: 'operator@x', role: 'operator' });
    const admin = await create.execute('admin', { email: 'admin@x', role: 'admin' });
