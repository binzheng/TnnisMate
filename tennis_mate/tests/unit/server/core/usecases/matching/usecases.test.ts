...
import {
	ActOnProposalUseCase,
	SendProposalUseCase,
	UpsertProfileUseCase,
} from "@/server/core/usecases/matching/usecases";

class Repo implements MatchingRepository {
	async upsertProfile(input: any) {
		return {
			userId: input.userId,
			level: input.level,
			area: input.area,
	...
	async updateProposalStatus(id: any, status: any) {
		return {
			id,
			fromUser: "f",
			toUser: "t",
...
describe("Matching usecases", () => {
	it("upsert profile logs audit", async () => {
		...
		expect(audit.events.some((e) => e.msg === "matching.profile.upsert")).toBe(
			true,
		);
	});
	...
	it("send proposal validates and logs", async () => {
		...
		expect(p.id).toBe("p1");
		expect(audit.events.some((e) => e.msg === "matching.proposal.send")).toBe(
			true,
		);
	});
	...
	it("act on proposal checks recipient and logs", async () => {
		...
		await expect(
			uc.execute({ userId: "wrong", id: "p1", action: "accept" }),
		).rejects.toThrow();
		const r = await uc.execute({ userId: "u2", id: "p1", action: "accept" });
		expect(r.status).toBe("accepted");
		expect(audit.events.some((e) => e.msg === "matching.proposal.act")).toBe(
			true,
		);
	});
});
