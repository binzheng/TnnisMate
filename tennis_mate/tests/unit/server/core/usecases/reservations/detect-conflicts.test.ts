...
describe('overlapsHalfOpen', () => {
  it('returns true when half-open ranges overlap', () => {
    expect(overlapsHalfOpen(new Date(0), new Date(10), new Date(5), new Date(15))).toBe(true);
  });
  it('returns false when touching at boundary', () => {
    expect(overlapsHalfOpen(new Date(0), new Date(10), new Date(10), new Date(15))).toBe(false);
  });
});
