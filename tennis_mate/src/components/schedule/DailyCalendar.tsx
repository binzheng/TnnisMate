...
export function DailyCalendar({
  ...
  onDragSelect?: (sel: { courtId: string; startHour: number; endHour: number }) => void;
  onDropEvent?: (
    eventId: string,
  ...
  const [dragging, setDragging] = useState<{ courtIndex: number; startHour: number } | null>(null);
  ...
  const cellBg = (courtIdx: number, hour: number) => {
    if (!dragging || dragging.courtIndex !== courtIdx || !overRef.current) return "background.default";
    return hour >= dragging.startHour && hour < overRef.current.endHour ? "action.selected" : "background.default";
  };
  ...
  const displayCourts = courts.length > 0 ? courts : [{ id: 'default', name: 'コート' }];

  return (
    <Box>
      {/* Date Navigation */}
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        <IconButton onClick={handlePrevDay} aria-label="前の日">
          <ChevronLeft />
        </IconButton>
        <Typography variant="h6">{formatDate(date)}</Typography>
        <IconButton onClick={handleNextDay} aria-label="次の日">
          <ChevronRight />
        </IconButton>
      </Stack>

      <Box sx={{ border: 1, borderColor: "divider", borderRadius: 1, overflow: "hidden" }} onMouseUp={handleUp}>
        {/* Header Row */}
        <Box
          sx={{
            display: "grid",
        ...
        >
          ...
          {displayCourts.map((court) => (
            <Box key={court.id} sx={{ p: 1, textAlign: "center", fontWeight: 600 }}>
              {court.name}
            </Box>
          ))}
        </Box>
        ...
        >
          {HOURS.map((h) => (
            <Box key={`row-${h}`} sx={{ display: "contents" }}>
              {/* Time label */}
              <Box sx={{ p: 1, borderRight: 1, borderColor: "divider", bgcolor: "background.paper" }}>
                ...
                <Box
                  ...
                  onDrop={(e) => {
                    ...
                    try {
                      ...
                      if (id) onDropEvent(id, { courtId: court.id, startHour: h, endHour: h + 1 });
                    } catch {}
                  }}
                >
                    ...
                    .map((e) => (
                      <Box
                        ...
                        sx={{
                          ...
                          left: 2,
                          right: 2,
                          height: Math.max(42, (e.endHour - e.startHour) * 48 - 4),
                          bgcolor: e.changed ? "warning.main" : e.needsReview ? "error.main" : "primary.main",
