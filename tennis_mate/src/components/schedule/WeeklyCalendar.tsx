...
export function WeeklyCalendar({
  ...
  onDragSelect?: (sel: { dayIndex: number; startHour: number; endHour: number }) => void;
  onDropEvent?: (
    eventId: string,
  ...
  const [dragging, setDragging] = useState<{ dayIndex: number; startHour: number } | null>(null);
  ...
  const handleUp = () => {
    if (dragging && overRef.current && onDragSelect) {
      onDragSelect({ dayIndex: dragging.dayIndex, startHour: dragging.startHour, endHour: overRef.current.endHour });
    }
    setDragging(null);
  ...
  const cellBg = (dayIdx: number, hour: number) => {
    if (!dragging || dragging.dayIndex !== dayIdx || !overRef.current) return "background.default";
              ...
              <Box
                ...
                onDrop={(e) => {
                  ...
                  try {
                    ...
                    if (id) onDropEvent(id, { dayIndex: di, startHour: h, endHour: h + 1 });
                  } catch {}
                }}
              >
                  ...
                  .map((e) => (
                    <Box
                      ...
                      sx={{
                        ...
                        left: 2,
                        right: 2,
                        height: Math.max(30, (e.endHour - e.startHour) * 36 - 4),
