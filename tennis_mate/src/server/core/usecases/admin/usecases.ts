...
export class KpiUseCase {
  ...
  async execute(q: KpiQuery = {}): Promise<KpiResult> {
    const [reservationsCount, lessonReservationsTotal, cancellationsCount, totalCapacity, avgPrice] = await Promise.all([
    ...
    const utilizationPercent = totalCapacity > 0 ? Math.min(100, Math.round((lessonReservationsTotal / totalCapacity) * 100)) : 0;
    const estimateYen = lessonReservationsTotal * (avgPrice || 2000);
    return { reservationsCount, lessonReservationsTotal, cancellationsCount, utilizationPercent, estimateYen };
  }
}
