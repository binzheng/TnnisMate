...
export class CreateSlotUseCase {
	...
	async execute(
		...
		this.audit?.log({
			level: "info",
			msg: "lesson.slot.create",
			userId,
			resource: `lessonSlot:${slot.id}`,
		});
		return slot;
	}
}
...
export class DeleteSlotUseCase {
	...
	async execute(userId: string, id: string): Promise<void> {
		...
		this.audit?.log({
			level: "info",
			msg: "lesson.slot.delete",
			userId,
			resource: `lessonSlot:${id}`,
		});
	}
}
...
export class SearchCandidateSlotsUseCase {
	...
	execute(q: SearchSlotsQuery) {
		return this.repo.searchCandidateSlots(q);
	}
}
...
export class BulkChangeReservationsUseCase {
	...
	async execute(
		...
		await this.authz.requireRole(userId, ["operator", "admin"]);
		const updated = await this.repo.bulkChangeReservations(
			reservationIds,
			targetSlotId,
		);
		this.audit?.log({
			level: "info",
			msg: "lesson.reservation.bulkChange",
			userId,
			resource: `lessonSlot:${targetSlotId}`,
			meta: { count: updated },
		});
		return updated;
	}
