...
export const usersRouter = createTRPCRouter({
  ...
  create: protectedProcedure
    .input(z.object({ email: z.string().email(), name: z.string().optional(), role: z.enum(["player","coach","operator","admin"]).optional(), password: z.string().min(6).optional() }))
  ...
  update: protectedProcedure
    .input(z.object({ id: z.string(), email: z.string().email().optional(), name: z.string().optional(), role: z.enum(["player","coach","operator","admin"]).optional() }))
    .mutation(async ({ ctx, input }) => {
      const uc = createUsersUseCases(ctx.db);
  ...
  resetPassword: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await uc.resetPassword.execute(ctx.session.user.id, { id: input.id, passwordHash });
      return { ok: true } as const;
    }),
});
