...
export const reservationsRouter = createTRPCRouter({
  ...
  createReservation: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      return uc.create.execute({ courtId: input.courtId, userId: ctx.session.user.id, start: input.start, end: input.end, requestId: ctx.requestId });
    }),

  updateReservation: protectedProcedure
    .input(
      z.object({ id: z.string(), start: z.coerce.date().optional(), end: z.coerce.date().optional() }),
    )
    .mutation(async ({ ctx, input }) => {
      const uc = createReservationUseCases(ctx.db);
      return uc.update.execute({ id: input.id, start: input.start, end: input.end, userId: ctx.session.user.id, requestId: ctx.requestId });
    }),
  ...
  cancelReservation: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const uc = createReservationUseCases(ctx.db);
      await uc.cancel.execute({ id: input.id, userId: ctx.session.user.id, requestId: ctx.requestId });
      return { ok: true };
    }),
});
