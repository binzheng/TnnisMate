	...
	implements AdminAnalyticsRepository
{
	...
	reservationsCount(q: KpiQuery): Promise<number> {
		return this.db.reservation.count({
			where: this.rangeFilter(q, ["start", "end"]),
		});
	}
	lessonReservationsTotal(q: KpiQuery): Promise<number> {
		return this.db.lessonReservation.count({
			where: this.rangeFilter(q, ["createdAt"]),
		});
	}
	cancellationsCount(q: KpiQuery): Promise<number> {
		return this.db.lessonReservation.count({
			where: {
				status: "cancelled",
				...(this.rangeFilter(q, ["createdAt"]) ?? {}),
			},
		});
	}
	async lessonSlotsCapacitySum(q: KpiQuery): Promise<number> {
		const agg = await this.db.lessonSlot.aggregate({
			_sum: { capacity: true },
			where: this.rangeFilter(q, ["start", "end"]),
		});
	...
	async lessonAveragePrice(): Promise<number> {
		const agg = await this.db.lessonPolicy.aggregate({
			_avg: { priceYen: true },
		});
