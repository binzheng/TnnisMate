...
import type { AddScoreInput, HistoryQuery, RankingRowDto, ScoreRecordDto, ScoreRepository } from "~/server/core/usecases/scores/ports";
...
export class PrismaScoreRepository implements ScoreRepository {
  ...
  async createScore(input: AddScoreInput): Promise<ScoreRecordDto> {
    const rec = await this.db.scoreRecord.create({ data: { playerId: input.playerId, opponentId: input.opponentId, date: input.date, result: input.result } });
  ...
  async findHistory(q: HistoryQuery): Promise<ScoreRecordDto[]> {
    ...
    const rows = await this.db.scoreRecord.findMany({ where, orderBy: { date: "desc" }, take: 100 });
    return rows as unknown as ScoreRecordDto[];
  }

  async groupRankingTop(limit: number): Promise<RankingRowDto[]> {
    const rows = await this.db.scoreRecord.groupBy({ by: ["playerId"], _count: { _all: true } });
    return rows
