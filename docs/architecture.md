...
バージョン: Postgres/Prisma の安定版（後で検証して明記）
検証日: （後で検証）
...
バージョン: NextAuth/Prisma 安定版（後で検証して明記）
検証日: （後で検証）
...
### Version Matrix (Provisional)

バージョンはオフライン環境のため暫定。セットアップ時に検証して固定し、検証日を更新する。

| Component | Version | Verification |
| --------- | ------- | ------------ |
| Node.js | 20.x LTS | 未検証（要確認） |
| Next.js | 14.x | 未検証（要確認） |
| TypeScript | 5.x | 未検証（要確認） |
| Prisma | 5.x | 未検証（要確認） |
| tRPC | 10.x | 未検証（要確認） |
| NextAuth.js | 4.x | 未検証（要確認） |
| Tailwind CSS | 3.x | 未検証（要確認） |
| UploadThing | 6.x | 未検証（要確認） |

Verification Date: ${ts}
